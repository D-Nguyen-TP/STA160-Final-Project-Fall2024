...
ggplot(dat, mapping = aes(x = Year, y = Temps)) + 
  ...
  labs(title = "Sacramento Average Temperatures Yearly", x = "Years", y = "Average Anomaly") #+ 
...
```{r}
series = ts(dat$Anomaly, start = dat$Year[1])
...
## Splitting the Data for comparison

```{r}
first = ts(full_raw$Temps[1:720], start = c(full_raw$Year[1], full_raw$Month), frequency = 12)
last = ts(full_raw$Temps[1584:2064], start = c(full_raw$Year[1584], full_raw$Month[1584]), frequency = 12)
...
```{r, eval=FALSE}
test = auto.arima(full, seasonal = T, D = 1)

autoplot(test)
```

```{r}
forecast(test, h = 24) %>% autoplot() + scale_x_continuous(limits = c(2000, 2022))
